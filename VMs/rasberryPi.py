...
while True:
    ...
    response = requests.post('https://api.zara4.com/oauth/access_token', data=data).json()
    AT = response['access_token']
    files = {
        'access_token': (None, response['access_token']),
        'file': ('Images/'+str(ranInd)+'.jpeg', open('Images/'+str(ranInd)+'.jpg', 'rb')),
    }
    response = requests.post('https://api.zara4.com/v1/image-processing/request', files=files).json()
    print(response)
    b64 = base64.b64encode(requests.get(response["generated-images"]["urls"][0]+'?access_token='+AT).content).decode('utf-8')
